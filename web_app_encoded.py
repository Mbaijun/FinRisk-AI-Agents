# web_app_encoded.py - 修复编码问题的Web应用
...
with st.sidebar:
    ...
    if check_api_connection():
        st.markdown('<div class="success-msg">✅ API服务已连接</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="error-msg">❌ API服务未连接</div>', unsafe_allow_html=True)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if risk_data:
                ...
                with col3:
                    st.metric("涨跌家数", f"{risk_data.get('advancers', 0)}/{risk_data.get('decliners', 0)}")
                with col4:
            ...
            with st.spinner("分析中..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        metric_config = [
                            ("波动率", "volatility", "{:.2%}", col1),
                            ("夏普比率", "sharpe_ratio", "{:.2f}", col2),
                            ("最大回撤", "max_drawdown", "{:.2%}", col3),
                            ("VaR (95%)", "var_95", "{:.2%}", col4),
                            ("CVaR (95%)", "cvar_95", "{:.2%}", col5)
                        ]
                        
                        for label, key, fmt, col in metric_config:
                            with col:
                        ...
                        if history.get("dates") and history.get("prices"):
                            df = pd.DataFrame({
                                "日期": history["dates"],
                                "价格": history["prices"]
                            })
    ...
    if not st.session_state.api_connected:
        ...
        if selected_stocks:
            ...
            if abs(total - 1.0) > 0.001:
                st.warning(f"权重总和: {total:.3f} (应为1.0)")
            else:
                st.success(f"✅ 权重总和: {total:.3f}")
            ...
            if analyze_port and abs(total - 1.0) <= 0.001:
                with st.spinner("分析组合中..."):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            col1, col2, col3, col4 = st.columns(4)
                            with col1:
                                st.metric("预期收益", f"{portfolio.get('expected_return', 0):.2%}")
                            with col2:
                                st.metric("波动率", f"{portfolio.get('volatility', 0):.2%}")
                            with col3:
                                st.metric("夏普比率", f"{portfolio.get('sharpe_ratio', 0):.2f}")
                            with col4:
                                st.metric("分散效益", f"{portfolio.get('diversification_benefit', 0):.2%}")
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if summary:
                            col1, col2 = st.columns(2)
                            with col1:
                                st.metric("风险等级", summary.get("risk_level", ""))
                            with col2:
                                st.metric("投资建议", summary.get("recommendation", ""))
                                ...
                                try:
                                    ...
                                    if export_response.status_code == 200:
                                        ...
                                        if export_format == "CSV":
                                            ...
                                            json_str = json.dumps(export_data.get("content", {}), indent=2, ensure_ascii=False)
                                            st.download_button(
                                                label="📥 下载JSON文件",
                                                data=json_str,
...
# 页脚
st.markdown("---")
st.markdown("© 2024 FinRisk AI Agents - 专业金融风险分析系统 | 版本 3.0")
