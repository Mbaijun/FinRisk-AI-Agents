...
if 'export_stock' not in st.session_state:
    st.session_state.export_stock = "AAPL"
if 'export_days' not in st.session_state:
    st.session_state.export_days = 30
...
# 独立的导出函数
def export_data_function(symbol, days, export_format):
    """导出数据的独立函数"""
    try:
        ...
        if response.status_code == 200:
            ...
            return {"error": f"导出失败: {response.text}"}
...
with st.sidebar:
    ...
    if st.button("📥 导出数据", key="sidebar_export_btn", use_container_width=True):
        with st.spinner("导出中..."):
            export_result = export_data_function(export_stock, export_days, export_format)
            ...
            if "error" not in export_result:
                if export_format == "CSV":
                    st.download_button(
                        ...
                        file_name=export_result.get("filename", "data.csv"),
                        mime="text/csv",
                        key="sidebar_download_csv"
                    )
                else:
                    json_content = json.dumps(export_result.get("content", {}), indent=2, ensure_ascii=False)
                    st.download_button(
                        label="📥 下载JSON文件",
                        data=json_content,
                        file_name=export_result.get("filename", "data.json"),
                        mime="application/json",
                        key="sidebar_download_json"
                    )
                
                st.success(f"✅ 数据已准备，大小: {export_result.get('size', 0)} 字节")
            else:
    ...
    if not st.session_state.api_connected:
        ...
        with col2:
            days = st.slider(
                ...
                value=30,
                step=7,
                key="single_days_slider"
            )
        with col3:
            st.write("")
            st.write("")
            analyze_btn = st.button("🚀 开始分析", type="primary", use_container_width=True, key="single_analyze_btn")
        ...
        if selected_stocks:
            ...
            with col1:
                days = st.slider(
                    ...
                    value=30,
                    step=7,
                    key="portfolio_days_slider"
                )
            with col2:
                st.write("")
                st.write("")
                analyze_port = st.button("🚀 分析组合", type="primary", use_container_width=True, key="portfolio_analyze_btn")
    ...
    if not st.session_state.api_connected:
        ...
        with col2:
            days = st.slider(
                ...
                value=30,
                step=7,
                key="report_days_slider"
            )
        with col3:
            st.write("")
            st.write("")
            generate_btn = st.button("📊 生成报告", type="primary", use_container_width=True, key="report_generate_btn")
            ...
            with st.spinner("生成报告中..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # 注意：导出功能现在在侧边栏中
                        st.info("💡 提示：数据导出功能已移至左侧边栏的'数据导出'面板")
        ...
        if st.session_state.current_report:
            st.markdown("---")
            st.subheader("📋 当前报告")
            st.json(st.session_state.current_report, expanded=False)
